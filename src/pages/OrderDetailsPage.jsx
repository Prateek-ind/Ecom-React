...
import { fetchSingleOrderFromDb } from "../features/order/orderAPI";
...
const OrderDetailsPage = () => {
  ...
  const {
    ...
    queryFn: () => fetchSingleOrderFromDb({ orderId, uid }),
    enabled: !!uid && !!orderId,
  });
  ...
  if (isError) {
    content = (
      <div className="mx-auto max-w-6xl px-4 md:px-8 mb-6 mt-28">
        <h2 className="text-lg text-center text-gray-700 tracking-widest mb-8 uppercase">
  ...
  return <section className="w-full  py-10 bg-[#feffec]">{content}</section>;
};
